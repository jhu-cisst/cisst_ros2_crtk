...
public:
    /*!  Constructor using an existing ROS node.  This constructor
      creates 2 mtsROSBridge using a high periodicity to reduce
      latency.  The first one, m_subscribers_bridge is used for all
      the subscribers added to this CRTK bridge.  It performs the
    ...
    inline std::shared_ptr<rclcpp::Node> node_handle_ptr(void) {
        return m_node_handle_ptr;
    }

protected:
    //! ros node
    std::shared_ptr<rclcpp::Node> m_node_handle_ptr = nullptr;
