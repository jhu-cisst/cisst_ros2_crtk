...
mts_ros_crtk_bridge_required::mts_ros_crtk_bridge_required(const std::string & _component_name,
    ...
    mtsROSBridge(_component_name, _period_in_seconds, _node_handle_ptr)
{
    init();
}
                                                               ...
                                                               const std::vector<std::string> & _write_events)
{
    ...
    for (auto & _command :  _write_commands) {
        ...
        if ((_crtk_command == "servo_jp")
            ...
            this->AddPublisherFromCommandWrite<prmPositionJointSet, sensor_msgs::msg::JointState>
                (_interface_name, _command, _ros_topic);
        } else  if (_crtk_command == "servo_jf") {
            this->AddPublisherFromCommandWrite<prmForceTorqueJointSet, sensor_msgs::msg::JointState>
                (_interface_name, _command, _ros_topic);
        } else if ((_crtk_command == "servo_cp")
                   || (_crtk_command == "servo_cr")
                   || (_crtk_command == "move_cp")
                   || (_crtk_command == "move_cr")) {
            this->AddPublisherFromCommandWrite<prmPositionCartesianSet, geometry_msgs::msg::PoseStamped>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "servo_cf") {
            this->AddPublisherFromCommandWrite<prmForceCartesianSet, geometry_msgs::msg::WrenchStamped>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "state_command") {
            this->AddPublisherFromCommandWrite<std::string, crtk_msgs::msg::StringStamped>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "use_gravity_compensation") {
            this->AddPublisherFromCommandWrite<bool, std_msgs::msg::Bool>
                (_interface_name, _command, _ros_topic);
        } else {
    ...
    for (auto & _command : _read_commands) {
        ...
        if ((_crtk_command == "measured_js")
            ...
            this->AddSubscriberToCommandRead<prmStateJoint, sensor_msgs::msg::JointState>
                (_interface_name, _command, _ros_topic);
        } else  if ((_crtk_command == "measured_cp")
                    || (_crtk_command == "setpoint_cp")) {
            this->AddSubscriberToCommandRead<prmPositionCartesianGet, geometry_msgs::msg::PoseStamped>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "measured_cv") {
            this->AddSubscriberToCommandRead<prmVelocityCartesianGet, geometry_msgs::msg::TwistStamped>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "measured_cf") {
            this->AddSubscriberToCommandRead<prmForceCartesianGet, geometry_msgs::msg::WrenchStamped>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "jacobian") {
            this->AddSubscriberToCommandRead<vctDoubleMat, std_msgs::msg::Float64MultiArray>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "operating_state") {
            this->AddSubscriberToCommandRead<prmOperatingState, crtk_msgs::msg::OperatingState>
                (_interface_name, _command, _ros_topic);
        } else if (_crtk_command == "period_statistics") {
            this->AddSubscriberToCommandRead<mtsIntervalStatistics, cisst_msgs::msg::IntervalStatistics>
                (_interface_name, _command, _ros_topic);
        } else {
    ...
    for (auto & _event : _write_events) {
        ...
        if (_crtk_command == "input_data") {
            this->AddSubscriberToEventWrite<prmInputData, sensor_msgs::msg::Joy>
                (_interface_name, _event, _ros_topic);
        } else if (_crtk_command == "Button") {
            this->AddSubscriberToEventWrite<prmEventButton, sensor_msgs::msg::Joy>
                (_interface_name, _event, _clean_namespace); // for buttons, we just use the interface name
        } else if (_crtk_command == "operating_state") {
            this->AddSubscriberToEventWrite<prmOperatingState, crtk_msgs::msg::OperatingState>
                (_interface_name, _event, _ros_topic);
        } else if (_crtk_command == "error") {
            this->AddSubscriberToEventWrite<mtsMessage, std_msgs::msg::String>
                (_interface_name, _event, _ros_topic);
